...
"[project]/src/app/api/cron/auto-fetch/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    try {
        ...
        // Get all accounts with auto-filter enabled that need processing
        const now = new Date();
        const { data: accounts, error } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from('roblox_accounts').select('id, cookie, account_name, auto_filter_interval, last_auto_fetch').eq('auto_filter_enabled', true).eq('is_active', true);
        if (error) {
        ...
        for (const account of accounts){
            ...
            if (account.last_auto_fetch) {
                const lastFetch = new Date(account.last_auto_fetch);
                const minutesSinceLastFetch = (now.getTime() - lastFetch.getTime()) / (1000 * 60);
                if (minutesSinceLastFetch < account.auto_filter_interval) {
                    console.log(`Skipping ${account.account_name}: only ${minutesSinceLastFetch.toFixed(1)} min since last fetch`);
                    continue;
                }
            }
            ...
            try {
                ...
                const requests = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$roblox$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getFriendRequests"])(account.cookie, proxy);
                if (requests.length === 0) {
                    // Update last fetch time even if no requests
                    await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from('roblox_accounts').update({
                        last_auto_fetch: now.toISOString()
                    }).eq('id', account.id);
                    results.push({
                        accountId: account.id,
                        accountName: account.account_name,
                ...
                // Enrich with user info and bot detection
                ...
                for (const req of requests){
                    ...
                    const botResult = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$bot$2d$detector$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["detectBot"])(req.id, req.name, userInfo);
                    const enriched = {
                        id: req.id,
                        username: req.name,
                ...
                // Save ALL requests to cache (including bots - no auto-decline)
                await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$friend$2d$request$2d$service$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["saveFriendRequests"])(account.id, enrichedRequests);
                // Update last fetch time
                await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from('roblox_accounts').update({
                    last_auto_fetch: now.toISOString()
                }).eq('id', account.id);
                const botCount = enrichedRequests.filter((r)=>r.isBot).length;
                results.push({
                    accountId: account.id,
                    accountName: account.account_name,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__cab31a7c._.js.map