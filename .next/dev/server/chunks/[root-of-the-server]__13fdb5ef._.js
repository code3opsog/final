...
"[project]/src/app/api/filter/add-followers/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
    ...
    const stream = new ReadableStream({
        async start (controller) {
            ...
            try {
                ...
                // Get account
                const { data: account } = await __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$supabase$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from('roblox_accounts').select('cookie, account_name, roblox_user_id').eq('id', accountId).eq('user_id', user.id).single();
                if (!account) {
                    sendEvent('error', {
                ...
                // Get CSRF token
                const csrfToken = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$roblox$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getCSRFToken"])(account.cookie, proxy);
                if (!csrfToken) {
                    sendEvent('error', {
                ...
                // Get authenticated user ID if not stored
                let robloxUserId = account.roblox_user_id;
                if (!robloxUserId) {
                    const authUser = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$roblox$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getAuthenticatedUser"])(account.cookie, proxy);
                    if (!authUser) {
                        sendEvent('error', {
                    ...
                    robloxUserId = String(authUser.id);
                }
                sendEvent('progress', {
                ...
                let followers;
                try {
                    followers = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$roblox$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getFollowers"])(parseInt(robloxUserId), account.cookie, proxy);
                } catch (followerError) {
                    console.error('Error fetching followers:', followerError);
                    sendEvent('error', {
                        message: `Failed to fetch followers: ${followerError instanceof Error ? followerError.message : String(followerError)}`
                    });
                    controller.close();
                    return;
                }
                ...
                sendEvent('progress', {
                    phase: `Found ${followers.length} followers`,
                    percent: 10,
                    added: 0,
                    skipped: 0,
                    failed: 0,
                ...
                for(let i = 0; i < followers.length; i++){
                    ...
                    const result = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$roblox$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sendFriendRequest"])(follower.id, account.cookie, csrfToken, proxy);
                    if (result.success) {
                        if (result.skipped) {
                            skipped++;
                        } else {
                    ...
                    const percent = 10 + Math.round((i + 1) / followers.length * 88);
                    sendEvent('progress', {
                        phase: `Adding followers... ${i + 1}/${followers.length}`,
                        percent,
                        added,
                        skipped,
                        failed,
                        current: follower.name,
                        total: followers.length
                    });
                    // Delay between requests
                    if (i < followers.length - 1) {
                        await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$rate$2d$limiter$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["sleep"])(delayBetweenRequests);
                    }
                }
                sendEvent('complete', {
                    message: `Added ${added} followers as friends`,
                    added,
                    skipped,
                    failed,
                    total: followers.length
                ...
                sendEvent('error', {
                    message: 'Failed to add followers'
                });
                controller.close();
            }
        }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__13fdb5ef._.js.map